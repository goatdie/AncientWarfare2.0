...
namespace Figurebox
{
    class FunctionHelper //失去天命后首都周围地减低轻微忠诚度，飞地直接独立或者加入其他国家 天命减少 国王的氏族绝嗣,（没有开国之君特质) 收复战争得先判断双方实力
    {
        ...
        public static Dictionary<string, Dictionary<string, Tuple<int, int, int>>> CityYearData =
            new Dictionary<string, Dictionary<string, Tuple<int, int, int>>>();
        ...
        public static bool is_chinese => LocalizedTextManager.instance.language == "cz" ||
                                         LocalizedTextManager.instance.language == "ch";
                    ...
                    foreach (Kingdom k in World.world.kingdoms.list_civs)
                    {
                        ...
                        if (tianmingBoolValue && k != maxKingdom)
                        {
                            ...
                            foreach (City city in cityListCopy)
                            {
                                ...
                                if (city != k.capital && city.leader != null &&
                                    !city.neighbours_cities.Contains(k.capital))
                                {
        ...
        public static bool HasRecapturedAllRecentTerritories(War war)
        {
            ...
            foreach (var city in storedDefenderCities[war])
            {
                ...
                {
                    if (entries[i - 1].Value.Item2 >= currentYear - 100 &&
                        entries[i - 1].Key.Split('-')[0] != attackerId)
                    {
                        if (entries[i].Key.Split('-')[0] == attackerId &&
        ...
        public static void CityAgePostfix(City __instance)
        {
            ...
            else if (kingdomCityData[cityId] != kingdomId)
            {
                ...
                {
                    CityYearData[cityId].Add(combinedKey,
                        new Tuple<int, int, int>(recaptureCount, yeardata, recaptureCount - 1));
                }
                else
                {
                    CityYearData[cityId][combinedKey] = new Tuple<int, int, int>(recaptureCount, yeardata,
                        CityYearData[cityId][combinedKey].Item3);
                }
            }
        }
        ...
        public static int CalculateKingdomValue(Kingdom k)
        {
            ...
            if (k.king != null)
            {
                stewardship = k.king.data.stewardship * 10;
            }
