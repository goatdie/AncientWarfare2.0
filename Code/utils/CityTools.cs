...
namespace Figurebox.Utils
{
	public static class CityTools
	{
		...
		{
			WorldLogMessage mandateofheaven = new WorldLogMessage("mandateofheavenMessage", pKingdom.name, pKingdom.king.getName(), null);
		...
		{
			WorldLogMessage mandateofheaven = new WorldLogMessage("losemandateofheavenMessage", pKingdom.name, pKingdom.king.getName(), null);
			//ColorAsset colorAsset = Reflection.GetField(typeof(Kingdom), pKingdom, "colorAsset") as ColorAsset;
			mandateofheaven.color_special1 = pKingdom.kingdomColor.getColorText();
			//mandateofheaven.color_special1 = new Color32(255, 217, 134, 255);
		...
		{
			WorldLogMessage mandateofheavenlose = new WorldLogMessage("losekingdommandateofheavenMessage", pKingdom.name, null);
			//ColorAsset colorAsset = Reflection.GetField(typeof(Kingdom), pKingdom, "colorAsset") as ColorAsset;
		...
		{
			WorldLogMessage warmandateofheaven = new WorldLogMessage("warmandateofheavenMessage", pAttacker.name, pDefender.name, null);
			//ColorAsset colorAsset = Reflection.GetField(typeof(Kingdom), pKingdom, "colorAsset") as ColorAsset;
		...
		{
			WorldLogMessage joinAnotherKingdom = new WorldLogMessage("joinanotherkingdomMessage", pMain.name, pTarget.name, null);
			//ColorAsset colorAsset = Reflection.GetField(typeof(Kingdom), pKingdom, "colorAsset") as ColorAsset;
		...
		{
			WorldLogMessage warreclaim = new WorldLogMessage("reclaimwarendMessage", pAttacker.name, pDefender.name, Winner.name);
			//ColorAsset colorAsset = Reflection.GetField(typeof(Kingdom), pKingdom, "colorAsset") as ColorAsset;
		...
		{
			WorldLogMessage vassalWarStart = new WorldLogMessage("vassalWarStartMessage", pAttacker.name, pDefender.name, null);
		...
		{
			WorldLogMessage vassalWarEnd = new WorldLogMessage("vassalWarEndMessage", pAttacker.name, pDefender.name, null);
		...
		{
			WorldLogMessage IndependenceWarEndMessage = new WorldLogMessage("IndependenceWarMessage", pAttacker.name, pDefender.name, null);
			...
			if (FunctionHelper.CityYearData.ContainsKey(cityId))
			{
				...
				for (int i = 1; i < entries.Count; i++)
				{
					...
					if (entries[i - 1].Value.Item2 >= startYear && entries[i - 1].Key.Split('-')[0] != kingdomId)
					{
						...
						if (entries[i].Key.Split('-')[0] == kingdomId && entries[i - 1].Value.Item2 - entries[i].Value.Item2 >= 60)
						{
