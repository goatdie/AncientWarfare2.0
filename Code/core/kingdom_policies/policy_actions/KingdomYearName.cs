...
public class KingdomYearName
{
    ...
    public static void Make_New_YearName(AW_Kingdom kingdom)
    {
        ...
        if (kingdom.addition_data.Title == AW_KingdomDataAddition.KingdomTitle.Emperor)
        {
            // 帝国级别的命名规则
        ...
        {
            // 其他等级的命名规则
            string singleCharTitle = AW_Kingdom.GetSingleCharacterTitle(kingdom.addition_data.Title); // 返回 "公"

            yearName = kingdom.name + singleCharTitle + kingdom.king.getNameCharacter();
        }

        kingdom.addition_data.year_name = yearName;
        kingdom.addition_data.year_start_timestamp = World.world.getCurWorldTime();
    ...
    public static void ShowTextKingdom_Title(MapText __instance, ref Kingdom pKingdom)
    {
        ...
        {
            // 获取爵位头衔
            string titleString = awKingdom.GetTitleString(awKingdom.addition_data.Title);
    ...
    public static void ShowTextKingdom_Postfix(MapText __instance, Kingdom pKingdom)
    {
        ...
        if (awKingdom == null || string.IsNullOrEmpty(awKingdom.addition_data.year_name))
        {
            return;
        }

        // 获取年号信息
        int yearcal = World.world.mapStats.getYearsSince(awKingdom.addition_data.year_start_timestamp);
        string nianhao = awKingdom.addition_data.year_name;
