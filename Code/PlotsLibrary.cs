...
namespace Figurebox
{
    public class MorePlots : PlotsLibrary
    {
        ...
        public override void init()
        {
            ...
            addPlotsToLocalizedLibrary("en", reclaim.id, "War of Reclaim", "$initiator_kingdom$ declared a reclaim war to $target_kingdom$");
            addPlotsToLocalizedLibrary("cz", reclaim.id, "收复战争", "$initiator_kingdom$向$target_kingdom$发起收复战争");
            ...
            reclaim.check_supporters = PlotsLibrary.checkMembersToRemoveDefault;
    ...
    !pPlot.initiator_kingdom.getAlliance().kingdoms_hashset.Contains(pPlot.target_kingdom))
            ...
            addPlotsToLocalizedLibrary("en", usurpation.id, "Usurpation", "$initiator_kingdom$'s $initiator_actor$ is attempting to usurp the throne.");
            addPlotsToLocalizedLibrary("cz", usurpation.id, "篡位", "$initiator_kingdom$的$initiator_actor$正在试图篡夺王位。");
            ...
            usurpation.check_other_plots = delegate (Actor pActor, Plot pPlot)
        {
            ...
            usurpation.check_supporters = delegate (Actor pActor, Plot pPlot)
                {
                    if (PlotsLibrary.checkMembersToRemoveDefault(pActor, pPlot))
                    {
                        return true;
                    }
                    Kingdom kingdom = pActor.kingdom;
            ...
            usurpation.check_launch = (PlotCheckerLaunch)((pActor, pKingdom) =>
                    {
                        ...
                        if (pActor.getAge() <= 17 && pKingdom == null && pActor.city == null) { Debug.Log("不是老资历之很多null"); return false; }
            ...
            usurpation.action = (PlotAction)(pPlot =>
   {
       ...
       return true;
            ...
            addPlotsToLocalizedLibrary("en", vassalWar.id, "Vassal War", "$initiator_kingdom$ declared a vassal war to $target_kingdom$");
            addPlotsToLocalizedLibrary("cz", vassalWar.id, "附庸战争", "$initiator_kingdom$向$target_kingdom$发起附庸战争");
            ...
            vassalWar.check_supporters = PlotsLibrary.checkMembersToRemoveDefault;
            ...
            addPlotsToLocalizedLibrary("en", absorbvassal.id, "Absorb Vassal", "$initiator_kingdom$ is attempting to absorb $target_kingdom$.");
            addPlotsToLocalizedLibrary("cz", absorbvassal.id, "吞并附庸", "$initiator_kingdom$正在试图吞并$target_kingdom$。");

            AssetManager.plots_library.add(absorbvassal);

            absorbvassal.check_supporters = PlotsLibrary.checkMembersToRemoveDefault;
            ...
            addPlotsToLocalizedLibrary("en", IndependenceWar.id, "Independence War", "$initiator_kingdom$ trying to Independence.");
            addPlotsToLocalizedLibrary("cz", IndependenceWar.id, "独立", "$initiator_kingdom$正在试图独立。");

            // 将 IndependenceWar 添加到 AssetManager 的 plots_library 列表中
            AssetManager.plots_library.add(IndependenceWar);


            IndependenceWar.check_supporters = PlotsLibrary.checkMembersToRemoveDefault;

            IndependenceWar.check_launch = delegate (Actor pActor, Kingdom pKingdom)
            {
                Kingdom lord = KingdomVassals.GetKingdom(pKingdom);
            ...
            IndependenceWar.action = delegate (Plot pPlot)
{
    ...
    foreach (Actor actor in pPlot.supporters)
    {
        ...
        if (kingdom.isAlive() && kingdom.countCities() != 0 && !kingdom.hasEnemies() && kingdom.king != null)
        {
            ...
            if (KingdomVassals.IsVassal(kingdom) && KingdomVassals.GetKingdom(kingdom) == pPlot.target_kingdom)
            {
    ...
    return true;
        ...
        private static void addPlotsToLocalizedLibrary(string pLanguage, string id, string name, string description)
        {
            ...
            if (pLanguage == language)
            {
                ...
                localizedText.Add("plot_" + id, name);
                localizedText.Add("plot_description_" + id, description);
            }
        }
